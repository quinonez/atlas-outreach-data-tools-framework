...
class WZAnalysis(Analysis.Analysis):
  ...
  def initialize(self):
      self.invMass           =  self.addHistogram("invMass",           ROOT.TH1D("invMass", "invMass;M [GeV]; Events", 20, 0,200))
      self.WtMass            =  self.addHistogram("WtMass",            ROOT.TH1D("WtMass", "WtMass; M_{T,W} [GeV]; Events", 20, 0, 200))
      ...
      self.hist_leptptc     =  self.addStandardHistogram("lep_ptcone30")
      self.hist_leptetc     =  self.addStandardHistogram("lep_etcone20")
  ...
  def analyze(self):
      ...
      if not (wLepton.tlv() + etmiss.tlv()).Mt() > 30: return False;
      ...
      self.WtMass.Fill((wLepton.tlv() + etmiss.tlv()).Mt(), weight)
      ...
      [self.hist_leptptc.Fill(lep.isoptcone30(), weight) for lep in goodLeptons]
      [self.hist_leptetc.Fill(lep.isoetcone20(), weight) for lep in goodLeptons]
