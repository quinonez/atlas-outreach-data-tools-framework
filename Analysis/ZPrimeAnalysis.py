...
class ZPrimeAnalysis(Analysis.Analysis):
  ...
  def initialize(self):
      self.WtMass            =  self.addHistogram("WtMass",            ROOT.TH1D("WtMass", "W Boson Transverse Mass; M_{T,W} [GeV]; Events", 20, 0, 200))
  ...
  def analyze(self):
      ...
      mTW = math.sqrt(2*lepton.pt()*etmiss.et()*(1-math.cos(lepton.tlv().DeltaPhi(etmiss.tlv()))));
      # mTW = (lepton.tlv() + etmiss.tlv()).Mt()
      ...
      # W boson histogram
      # self.WtMass.Fill((lepton.tlv() + etmiss.tlv()).Mt(), weight)
      ...
      self.hist_leptptc.Fill(lepton.isoptcone30(), weight)
      self.hist_leptetc.Fill(lepton.isoetcone20(), weight)
