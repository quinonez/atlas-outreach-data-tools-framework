...
class HWWAnalysis(Analysis.Analysis):
  ...
  def initialize(self):
      ...
      self.hist_ptLL            = self.addHistogram("ptll",               ROOT.TH1D("ptll", "Tranvsere Momentum of Dilepton System; pt_{ll};Events", 20,0,200))
      self.hist_deltaPhiLL      = self.addHistogram("deltaphill",         ROOT.TH1D("deltaphill", "Azimuthal Opening Angle between Leptons; #Delta#phi_{ll};Events", 16, 0, 3.2))
  ...
  def analyze(self):
      ...
      self.hist_deltaPhiLL.Fill(leadLepton.tlv().DeltaPhi(trailLepton.tlv()))
