...
class MainDepiction(Depiction):
    ...
    def drawDepiction(self, paintables):
        ...
        for paintable in paintablesToPaint:
            ...
            option = "same"
        self.drawLegend(paintablesToPaint)

    def drawLegend(self, paintables):
       self.legend = ROOT.TLegend(0.75,0.85,0.9,0.65)       # Legend has to be attached to mainDepiction object 
       for paintable in paintables:                         # to avoid destruction by Garbage Collector
           paintable.addToLegend(self.legend)
